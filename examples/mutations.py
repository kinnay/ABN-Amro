...
async def main():
	...
	print("Authorizing...")
	await authorization.login(ACCOUNT_NUMBER, CARD_NUMBER, PASSWORD)
	print()

	print("Accounts:")
	product_groups = ["PAYMENT_ACCOUNTS", "SAVINGS_ACCOUNTS"]
	contracts = await accounts.list(product_groups=product_groups)
	...
	for i in range(3):
		list = await mutations.search(iban, page_token=page_token)
